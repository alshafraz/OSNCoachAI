// src/application/content-generation/services/generators/HintGenerator.ts
...
export class HintGenerator {
  ...
  async generate(request: GenerationRequest): Promise<GeneratedContent> {
    ...
    const response = await this.llmProvider.generate({
      ...
      temperature: rendered.temperature ?? acgpConfig.hintTemperature,
    ...
    // Validate progressive structure of hints
    const hints = body.hints ?? [];
    if (hints.length < 5) {
      // Pad default levels if generation missed any
      const hintTypes = ['observation', 'concept', 'formula', 'partialReasoning', 'nearComplete'];
      for (let i = hints.length; i < 5; i++) {
        hints.push({
          level: (i + 1) as any,
          type: hintTypes[i],
          text: `Progressive hint level ${i + 1} for solving this problem.`
        });
      }
    }
    ...
    const content: GeneratedContent = {
      ...
      createdAt: new Date(),
      updatedAt: new Date(),
      promptVersion: acgpConfig.promptVersions.hint,
